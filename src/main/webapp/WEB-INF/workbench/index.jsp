            ...
            <div class="modal-body">
                <form class="form-horizontal" role="form" id="updatePwdForm"
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-10" style="width: 300px;">
                            <input type="password" class="form-control" name="loginPwd" id="newPwd" style="width: 200%;">
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        <label for="oldPwd" class="col-sm-2 control-label">上传头像</label>
                        <a href="javascript:;" style="position: relative;left:15px" class="a-upload">
            ...
            <li class="dropdown user-dropdown">
                <a href="javascript:void(0)" style="text-decoration: none; color: white;" class="dropdown-toggle"
                   data-toggle="dropdown">
                    <img src="${sessionScope.user.photo}" style="width: 20px;height: 20px;border-radius: 100%" /> ${sessionScope.user.name} <span class="caret"></span>
                </a>
...
<script>
    //从原始密码输入框失去焦点的时候，发送异步请求，校验用户输入的原始密码是否正确
    $('#oldPwd').blur(function () {
        $.ajax({
            url : '/crm/user/verifyPwd',
            data : {'oldPwd':$(this).val(),'id':'${sessionScope.user.id}'},
            type : 'get',
            dataType : 'json',
    ...
    /**
     * 1、如果我们前台想以弹窗的形式返回信息，必须异步提交表单
     * 2、上传图片在选中图片之后就要把图片保存在服务器端
     */
    ...
    function updatePwd() {
        $.ajax({
            ...
            data : {'oldPwd':$('#newPwd').val(),'id':'${sessionScope.user.id}'},
            type : 'get',
            dataType : 'json',
            success : function(data){
                console.log(data);
    ...
    //用户退出系统
    function loginOut() {
        location.href = "/crm/user/loginOut";
    }
</script>
</html>